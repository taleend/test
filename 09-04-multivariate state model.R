...
#----model simulation------------------
#Gaussian likelihood with obs error variance
likelihood <- 'dnorm'
obsE <- .3 #obs error sd, logit scale
yobs <- matrix(rnorm(length(y),y,obsE),nrow(y),ncol(y))
mus <- .3 #mean obs error
mw <- nt*n*ns
obsError <- mus
z <- y2zMVlogit(yobs) #simulated proportions from observed
tmp <- initialStatesSS('dnorm',yobs,mus,mw)
yg <- tmp$x
s1 <- tmp$s1s2[1]
s2 <- tmp$s1s2[2]


prior.W <- diag(1,ns) #prior covariance matrix
prior.WDF <- p + n*ns*nt
pB <- matrix(0,p,ns)
